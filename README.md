...
- **Rankings tab** — a simplified, scannable list: each row shows a player's combined rank average on the left (not a list position — it's a fixed per-player stat), their name, and a Draft button. Rows alternate a lighter shade for readability. Shows a combined average ranking across whichever sources you select — a player's average only counts the sources that actually ranked them. Tap a player to see their full detail view; hit "Draft" to hide them everywhere (toggle "Include drafted players" to bring them back into view).
- **Draft List tab** — your own editable, drag-to-reorder list, shown the same way (combined rank average on the left, alternating row shades). It's seeded from the combined average the first time you use it, then persists independently — editing sources never silently overwrites it.
  - A list filter (checkboxes, same as Rankings) controls which sources feed both the average shown next to each name and the Reset button. A position filter (PG/SG/SF/PF/C, pick as many as you want) narrows the list to players eligible at any of the checked positions.
  - Hit "Reset" any time to resync unlocked players to that filtered average — no confirmation, it's easy to undo by dragging things back. Hit the 🔓/🔒 button on a player to lock them in place — locked players don't move on reset and can't be dragged, everyone else refills around them. "Unlock All" (with a confirmation) clears every lock at once.
...
Tapping a player opens a detail view with their combined rank average (based on whichever sources are selected in the tab you opened it from) as its own row up top, then one square per source — every source you've added, not just the special avg/total ones. Each square shows that source's rank in big text, with the actual stat (if that source has one, e.g. an avg or total) in small text underneath; squares for sources currently selected in your list filter are highlighted blue. At the bottom: "Mark Drafted"/"Undraft" plus "Drafted by me"/"Remove from My Team" — the latter both marks them drafted and adds them to the My Team tab in one step.
