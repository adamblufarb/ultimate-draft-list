...
- **Rankings tab** — a simplified, scannable list: each row shows a player's combined rank average on the left (not a list position — it's a fixed per-player stat), their name, their position(s) in a small badge, and — if tagged (see Player detail below) — their Breakout/Sleeper/Do Not Draft emoji on the right. Rows alternate a lighter shade for readability, and a small "— 10 —" / "— 20 —" divider marks every 10th player so you can gauge depth at a glance. Shows a combined average ranking across whichever sources you select — a player's average only counts the sources that actually ranked them. Tap a player to see their full detail view (that's also where you mark them drafted); tap "Include Drafted Players" to bring drafted players back into view.
