...
- **Rankings tab** — a simplified, scannable list: each row shows a player's combined rank average on the left (not a list position — it's a fixed per-player stat), their name, and a Draft button. Rows alternate a lighter shade for readability. Shows a combined average ranking across whichever sources you select — a player's average only counts the sources that actually ranked them. Tap a player to see their full detail view; tap "Include Drafted Players" to bring drafted players back into view.
- **Draft List tab** — your own editable, drag-to-reorder list, shown the same way (combined rank average on the left, alternating row shades). It's seeded from the combined average the first time you use it, then persists independently — editing sources never silently overwrites it.
  ...
  - Each position chip also shows how many of the best remaining undrafted players hold that position — e.g. "PG 4" — scoped to a pool size you pick from the dropdown next to the chips (10/20/30/.../100). That count re-ranks live off the current source filter, so it always reflects whichever sources are checked. A position's chip outlines orange once its share of the pool drops below 20%, and red below 15%, so you can see a position running thin before you're stuck.
  - "Unlock All" sits next to "Include Drafted Players" — tap it (with a confirmation) to clear every lock at once. Hit the 🔓/🔒 button on a player to lock them in place — locked players don't move when the filter resyncs the list and can't be dragged, everyone else refills around them.
